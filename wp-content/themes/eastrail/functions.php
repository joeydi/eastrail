...
class ET
{
  ...
  function action_enqueue_styles()
  {
    ...
    wp_enqueue_style('gfonts', 'https://fonts.googleapis.com/css2?family=Source+Sans+Pro:ital,wght@0,300;0,400;0,600;0,700;1,400;1,700&display=swap', false, $this->version);
  }
...
<?php
  ...
  static function video_embed($url)
  {
    $embed = wp_oembed_get($url);

    // Customize player parameters for Vimeo iframe src attribute
    $embed = preg_replace_callback(
      ...
      function ($matches) {
        ...
        if (strpos($url, 'vimeo') !== false) {
          ...
          $url = $url . $separator . 'title=0&byline=0&portrait=0&vimeo_logo=0';
        }
